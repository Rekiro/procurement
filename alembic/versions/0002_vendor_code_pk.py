"""Replace vendor UUID PK with vendor_code string PK

Revision ID: 0002
Revises: 0001
Create Date: 2026-03-14

Drops and recreates all tables that reference proc_vendors to switch
from UUID-based vendor_id to string-based vendor_code FK.
proc_api_logs is untouched.
"""
...
def upgrade() -> None:
    # Drop all tables that reference proc_vendors (reverse dependency order)
    ...
    # --- Recreate proc_vendors with vendor_code as PK ---
    op.create_table(
        "proc_vendors",
        sa.Column("vendor_code", sa.String(50), primary_key=True),
    ...
    # --- proc_vendor_applications with SERIAL PK + vendor_code FK ---
    op.create_table(
        "proc_vendor_applications",
        sa.Column("id", sa.Integer, primary_key=True, autoincrement=True),
        sa.Column("vendor_code", sa.String(50), sa.ForeignKey("proc_vendors.vendor_code"), nullable=False),
    ...
    # --- Recreate remaining tables with vendor_code instead of vendor_id ---
    ...
    op.create_table(
        ...
        sa.Column("vendor_code", sa.String(50), sa.ForeignKey("proc_vendors.vendor_code"), nullable=True),
        sa.Column("site_id", sa.String(100), nullable=False),
